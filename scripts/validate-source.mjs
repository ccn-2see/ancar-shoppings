...
assert(overview.includes('data-ancar-ui-version="5.8.8"'),'V5.8.8: marcador da home ausente');
assert(rootRoute.includes('ancar-ui=5.8.8')&&rootRoute.includes('content: "5.8.8"'),'V5.8.8: versão/cache bust ausente');
...
assert(settings.includes('Meta CAG')&&settings.includes('Meta Chillers')&&settings.includes('Tarifa de energia')&&settings.includes('Fator de emissão'),'Configurações: campos de energia, metas e fator de emissão ausentes');
for(const forbidden of ['Baseline kW/TR','Desvio máximo de balanço','Periféricos — atenção','Dados desatualizados após']) assert(!settings.includes(forbidden),`Configurações: campo não esperado ainda visível: ${forbidden}`);
...
assert(energy.includes('Custo energético')&&energy.includes('Custo acima da meta')&&energy.includes('Emissões associadas')&&energy.includes('Energia evitada vs. referência')&&energy.includes('Intensidade de carbono'),'Energia & Carbono: KPIs econômicos/carbono ausentes');
...
assert(config.includes('REFRESH_INTERVAL_MS = 5 * 60 * 1000'),'Polling deve acompanhar a coleta em 5 min');
...
console.log('VALIDAÇÃO DE FONTE V5.8.8: PASS');
