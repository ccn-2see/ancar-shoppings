...
export function ShoppingCard({ shopping }: { shopping: Shopping }) {
  ...
  const storageState = getThermalStorageDisplayState(shopping);
  const theme = storageState ?? statusTheme[performance.status];
  const statusLabel = storageState?.label ?? performance.label;
  const statusDotColor = storageState?.color ?? performance.dotColor;
  ...
  return (
    <Link to="/shoppings/$shoppingId" params={{ shoppingId: shopping.id }} aria-label={`Abrir ${shopping.name}`} title={shopping.name}
      ...
      <span className="absolute right-3 top-3 h-2.5 w-2.5 rounded-full shadow-[0_0_12px_currentColor]" style={{ color: statusDotColor, background: statusDotColor }} aria-label={statusLabel}/>
      ...
      <div className="mt-auto rounded-md border px-2 py-1 text-center text-[10px] font-semibold" style={{ color: theme.color, background: theme.background, borderColor: theme.border }}>{statusLabel}</div>
    </Link>
  );
}
