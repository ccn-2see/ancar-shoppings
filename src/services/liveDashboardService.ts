...
export const liveDashboardService = {
  ...
  async saveSettings(code: string, settings: ShoppingSettings): Promise<SettingsApiResponse> {
    const result = await requestJson<SettingsApiResponse>(`${API_BASE_URL}/ancar-settings-v1`, {
      method: "PUT",
      body: JSON.stringify({ shoppingId: code, targetKwTr: settings.targetKwTr, targetChillerKwTr: settings.targetChillerKwTr, energyTariffBrlMwh: settings.energyTariffBrlMwh, emissionFactorKgCo2Kwh: settings.emissionFactorKgCo2Kwh ?? 0.0461, updatedBy: "frontend" }),
    });
...
export function normalizeSettings(input?: Partial<ShoppingSettings> | null): ShoppingSettings {
  const emissionFactorKgCo2Kwh = asNumber(input?.emissionFactorKgCo2Kwh) ?? 0.0461;
  const usesDefaultEmissionFactor = Math.abs(emissionFactorKgCo2Kwh - 0.0461) < 1e-12;
  return {
    ...
    emissionFactorSource: input?.emissionFactorSource ?? (usesDefaultEmissionFactor ? "MCTI/SIRENE - Fator Médio de Emissão do SIN" : "Fator customizado pelo cliente"),
    emissionFactorReferenceYear: asNumber(input?.emissionFactorReferenceYear) ?? (usesDefaultEmissionFactor ? 2025 : null),
