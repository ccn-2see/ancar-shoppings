...
function ShoppingsPage() {
  ...
  const filtered = useMemo(() => list.filter((shopping) => {
    ...
    if (status !== "Todos" && getShoppingPerformanceState(shopping).status !== status) return false;
  ...
  const summary = useMemo(() => {
    ...
    const attention = list.filter((s) => ["atencao", "critico"].includes(getShoppingPerformanceState(s).status)).length;
  ...
  return <InternalPage className="compact-page compact-shoppings-page">
    ...
    <FilterBar>
      ...
      <Select label="Status" value={status} onChange={setStatus} options={STATUS_OPTS} labelMap={{otimo:"Ótimo",bom:"Bom",atencao:"Atenção",critico:"Crítico",offline:"Desligado / sem medição / sem meta"}}/>
